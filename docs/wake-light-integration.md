...
`home-assistant/custom_components/wake_light/` is a config-entry custom
...
The integration is published through the custom HACS repository `SFenton/ha-wake-light`. Installing or updating files still requires a Home Assistant restart before the new Python modules are active.
...
For the Master Bedroom, the live blocker is
`switch.adaptive_lighting_master_bedroom_lighting`. The lifecycle owner is
`automation.master_suite_bedtime_preparation_lifecycle`; its brightness-255
path must ignore active/recovering wake leases, and its clean wake handoff must
match only the PBL `released` / `completed` / `hold_complete` transition.

## Install and configure

### HACS

1. Add `SFenton/ha-wake-light` as a custom HACS integration repository.
2. Download **Wake Light** in HACS.
3. Restart Home Assistant.

For manual installation, copy `home-assistant/custom_components/wake_light/` to `<HA config>/custom_components/wake_light/` and restart Home Assistant.

The copied directory must contain `manifest.json`, `__init__.py`,
`config_flow.py`, `coordinator.py`, `sensor.py`, `service.py`, `services.yaml`,
`strings.json`, `translations/`, and the pure model/scheduler modules. Restart
Home Assistant so the new custom integration is discovered, then:

1. Open **Settings → Devices & services → Add integration**.
2. Select **Wake Light**.
3. Create one entry per room.
4. Supply a stable lowercase profile ID. The Master Bedroom React contract
   expects `master-bedroom`.
5. Select the root group, explicit leaf targets, PBL switch, required vacation
   entity, optional occupancy entity, and fail-closed blockers.
6. Optionally select the SleepyPod schedule sensor, allowed source sides, and
   matching side alarm-state sensors.
7. Keep the legacy brightness lifecycle acknowledgement off until its external
   automation is safe.
8. Complete setup and confirm the generated summary entity ID. For the profile
   name `Master Bedroom`, the suggested entity ID is
   `sensor.master_bedroom_wake_light`.

### Master Bedroom profile evidence

Use the current source-backed IDs:

| Field | Value |
| --- | --- |
| Profile ID | `master-bedroom` |
| Profile name | `Master Bedroom` |
| Root | `light.master_bedroom` |
| Occupancy (advisory) | `binary_sensor.master_bedroom_occupancy_sensors` |
| PBL switch | `switch.master_bedroom_presence_master_bedroom_presence_allowed` |
| Vacation | `input_boolean.vacation_mode` |
| Adaptive Lighting blocker | `switch.adaptive_lighting_master_bedroom_lighting` |
| SleepyPod schedule | `sensor.master_bedroom_sleepypod_eight_pod_schedules` |
| Left lifecycle | `sensor.master_bedroom_sleepypod_eight_pod_left_alarm_state` |
| Right lifecycle | `sensor.master_bedroom_sleepypod_eight_pod_right_alarm_state` |

The explicit wake-owned target list is:
...
Do not select all root members. In particular,
`light.master_bedroom_bathroom_light` is excluded. The closet light is also
outside the initial four-leaf wake profile.

The Adaptive Lighting blocker must be added after its current live entity ID is
confirmed.
...
is bounded by the PBL two-hour lease ceiling while accounting for the longest
loadable legacy 60-minute ramp, the configured post-wake hold, the
source-snooze budget, and recovery. New configuration writes accept 0, 5, 10,
15, or 30 ramp minutes and 5, 10, 15, or 30 post-wake hold minutes. Lease
updates never extend the initial absolute expiry.
...
For a dated one-time alarm, `bed_sides` may contain `left`, `right`, or both.
Home Assistant adds a temporary SleepyPod alarm only when that side does not
already have the selected date's execution weekday/time. It records the
provider IDs that existed before the write, binds the newly assigned ID after
the schedule sensor updates, and removes only that proven-new alarm ten minutes
after the dated wake time. The pending cleanup is persisted and retried after a
restart or temporary schedule-sensor outage. Weekly alarms continue to use the
per-source link and schedule synchronization path.
...
`alarm_links` records only explicit opt-outs. Every source alarm day drives the
room wake lights unless its link key appears here with `false`. A link key is
`<source_ref>#<weekday>#<HH:MM>`, derived from the source side, the execution
weekday and the local wake time, so Home Assistant and the dashboard resolve the
same key without a source-owned row identifier. One `link_alarm` command carries
every execution-day key of a single grouped source alarm, so a dashboard toggle
is one command and one revision. Changing an alarm's day or time
in the source device produces a new key and therefore restores the default
linked state.
