...
| Text-entry focus and overflow | Pointer click/tap keeps input and wrapper outline, border, and box-shadow unchanged on coarse and fine pointers; keyboard Tab retains identifiable caret/selection or intentional keyboard focus, and errors retain their borders. Long multiline and unbroken pasted input stays scrollable/editable with caret reveal, hidden scrollbar chrome, and stable geometry. | Focused mocked Playwright click, tap, Tab, computed-style comparisons, overflow/end-scroll/edit checks and directly viewed screenshots in Chromium and WebKit. Static scanners alone do not certify native control behavior or physical iOS scroll indicators. |
