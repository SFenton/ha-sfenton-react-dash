...
export const MANUAL_NEW_SURFACE_GUIDE_ARTICLES: ManualSurfaceGuideArticle[] = [
  {
    ...
    surfaceGuide: {
      howToOpen: [
        'Open Climate, review the Whole Home dial and Thermostat Hub, then select Thermostat Controls under Rooms & Settings.',
      ...
      contents: [
        'Rooms lists every configured virtual room thermostat with its current temperature and occupancy summary. Selecting a room shows its dial, vent status, and any effective Away or Vacation notice.',
        'Automation starts with the Automatic Thermostat master, followed by Eco Mode policy and Predictive Comfort. Critical Tracking, Away Behavior, and Predictive explanation open as focused pages inside the same sheet.',
        'Tracking keeps batch room decisions grouped by policy. Selected Rooms, Critical Protection, and Occupied Only each open a complete room checklist while Home Assistant remains the source of truth.',
      ],
      navigation: {
        ...
        tabs: [
          item('climate.thermostat-tab-rooms', 'Rooms', 'Lists every configured room and opens the selected room thermostat detail without changing a target merely by navigating.'),
          item('climate.thermostat-tab-automation', 'Automation', 'Contains the master integration control, Eco policy, and Predictive Comfort entry points in dependency order.'),
          item('climate.thermostat-tab-tracking', 'Tracking', 'Groups the selected, critical-protection, and occupancy-only room policies into batch-oriented child pages.'),
        ],
        detailPages: [
          ...
          item('climate.eco-critical-tracking-detail', 'Eco Mode Critical Tracking', 'Selects whether no rooms, selected untracked rooms, or all rooms receive critical-temperature protection.'),
          item('climate.eco-away-behavior-detail', 'Eco Behavior When Away', 'Selects whether Eco disables, uses the away target range, or remains active when everyone is away.'),
          item('climate.tracking-selected-rooms-detail', 'Selected Rooms', 'Edits the batch list of rooms that participate when selected-room tracking is enabled.'),
          item('climate.tracking-critical-protection-detail', 'Critical Protection', 'Edits eligible unselected rooms only when the configured critical policy allows a selective list.'),
          item('climate.tracking-occupied-only-detail', 'Occupied Only', 'Edits the rooms that leave demand and minimum-vent balancing while empty and rejoin when occupied.'),
        ],
        wizardSteps: [],
      },
      ...
      troubleshootingChecks: [
        ...
        'If Back closes instead of returning to a tab, reopen Thermostat Controls and confirm that a child page was entered from the tab overview.',
      ],
      screenshotIds: [
