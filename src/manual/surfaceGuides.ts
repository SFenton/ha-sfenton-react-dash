...
export const MANUAL_NEW_SURFACE_GUIDE_ARTICLES: ManualSurfaceGuideArticle[] = [
  {
    ...
    visibleLabels: ['Thermostat · Advanced Controls', 'Room Thermostats', 'Advanced Configuration', 'Room Tracking', 'Rooms', 'Automation', 'Tracking', 'Selected Rooms', 'Critical Protection', 'Occupied Only'],
    ...
    surfaceGuide: {
      ...
      contents: [
        'The root sheet uses the title Thermostat · Advanced Controls without a header subtitle. Nested pages use their focused page title; Predictive Comfort may also show its current recommendation as page-specific context.',
        'Rooms uses full-size colored glass navigation tiles that show current temperature and occupancy. Selecting a room shows its dial, vent status, and any effective Away or Vacation notice.',
        'Automation uses full-size app-standard green or grey glass toggle tiles for on and off state. Full-size colored glass navigation tiles open Critical Tracking, Away Behavior, and Predictive Comfort inside the same sheet after a short explanation.',
        'Tracking explains each batch rule before its full-size colored glass navigation tile. Selected Rooms, Critical Protection, and Occupied Only each open a dynamic room grid while Home Assistant remains the source of truth.',
      ],
      navigation: {
        ...
        tabs: [
          ...
          item('climate.thermostat-tab-automation', 'Automation', 'Explains the master integration, Eco policy, and Predictive Comfort controls before each full-size green or grey toggle tile and colored navigation tile.'),
        ...
        detailPages: [
          ...
          item('climate.eco-critical-tracking-detail', 'Eco Mode Critical Tracking', 'Uses a vertical stack with a short description above each subtitle-free, full-size green or grey GlassTile. Choosing a protection policy updates the selection in place and keeps this detail page open.'),
          item('climate.eco-away-behavior-detail', 'Eco Behavior When Away', 'Uses a vertical stack with a short description above each subtitle-free, full-size green or grey GlassTile. Choosing an away policy updates the selection in place and keeps this detail page open.'),
      ...
      closeBackCancelBehavior: 'Changing an Eco option stays on its detail page so another policy can be reviewed; Back explicitly returns to Automation. Back from other detail pages returns to their owning tab. X, backdrop, swipe, Escape, browser Back, or clearing the hash closes the complete sheet through the shared exit animation. Closing does not undo a target, mode, option, or toggle that Home Assistant already accepted.',
