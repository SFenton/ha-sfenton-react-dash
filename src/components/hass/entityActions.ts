...
export function resolveEntityAction<TAction extends EntityBasicAction>(entityId: string, action: TAction | EntityStateAction<TAction> | undefined, entities: EntityActionStateMap) {
  if (!action) return undefined
  if (action.type !== 'state') return action

  const stateEntityId = action.entityId ?? entityId
  const state = entities[stateEntityId]?.state
