...
describe('CircularControlDial', () => {
  ...
  it('renders ordered static markers without slider semantics', () => {
    ...
    expect(markers[0]).toHaveAttribute('data-marker', 'current')
    expect(markers[0]).toHaveAttribute('data-value', '-2')
    expect(markers[1]).toHaveAttribute('data-marker', 'target')
    expect(markers[1]).toHaveAttribute('data-value', '-10')
