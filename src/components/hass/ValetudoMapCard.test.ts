...
describe('ValetudoMapCard helpers', () => {
  ...
  it('derives bounds from rendered pixels instead of explicit empty layer dimensions', () => {
  ...
  it('ignores dimensions-only metadata when drawable pixels exist', () => {
    const map = mapWithLayers([
      {
        type: 'segment',
      ...
      {
        type: 'metadata',
        dimensions: { x: { min: -500, max: 900 }, y: { min: -800, max: 1200 } },
      },
    ])

    expect(valetudoMapBounds(map)).toEqual({ minX: 10, maxX: 12, minY: 20, maxY: 20 })
  })
  ...
  it('uses valid dimensions-only geometry when the whole scene has no drawable pixels', () => {
    const map = mapWithLayers([
      {
        type: 'metadata',
        dimensions: { x: { min: -3, max: -1 }, y: { min: 4, max: 9 } },
      },
      {
        type: 'metadata',
        dimensions: { x: { min: 10, max: 12 }, y: { min: -5, max: 1 } },
      },
    ])

    expect(valetudoMapBounds(map)).toEqual({ minX: -3, maxX: 12, minY: -5, maxY: 9 })
  })

  it('falls back to map size when no drawable or valid legacy geometry exists', () => {
    const map = mapWithLayers([
      {
        type: 'floor',
        dimensions: { x: { min: -500, max: 900 }, y: { min: -800, max: 1200 } },
      ...
      {
        type: 'metadata',
        dimensions: { x: { min: 6, max: 2 }, y: { min: 3, max: Number.NaN } },
      },
    ])
