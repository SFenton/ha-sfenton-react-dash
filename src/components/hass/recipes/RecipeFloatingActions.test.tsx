...
describe('RecipeFloatingActions', () => {
  it('uses approved defaults and commits sort drafts only on Apply', async () => {
    ...
    expect(within(filterDialog).getByText('Weights are not used while recipes are sorted alphabetically.')).toBeInTheDocument()
  ...
  it('keeps independent filter drafts, commits only on Apply, and resets drafts', async () => {
    ...
    const expiryToggle = within(filterDialog).getByLabelText('Turn on Expiring ingredients only')
    fireEvent.click(expiryToggle)
    fireEvent.click(within(filterDialog).getByRole('radio', { name: '30 Days' }))
    fireEvent.change(within(filterDialog).getByRole('slider', { name: 'Minimum Coverage' }), { target: { value: '45' } })
    ...
    expect(within(filterDialog).getByRole('slider', { name: 'Minimum Coverage' })).toHaveValue('0')
