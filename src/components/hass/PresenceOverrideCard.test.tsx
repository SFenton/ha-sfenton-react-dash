...
describe('PresenceOverrideCard', () => {
  ...
  it('uses a dynamic Alarm-style two-column grid and optimistically applies state', () => {
    ...
    expect(stateGrid).toHaveAttribute('data-dynamic-grid-item-sizing', 'uniform')
    ...
    const enabledButton = screen.getByRole('button', { name: 'Set Test Room presence lighting to Enable Presence-Based Lighting' })
    expect(enabledButton).toHaveAttribute('data-active', 'true')
    expect(enabledButton).toHaveAttribute('data-icon-surface', 'false')
    expect(enabledButton).not.toHaveAttribute('data-wrap-text')
