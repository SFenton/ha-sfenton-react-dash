...
describe('AtAGlancePage', () => {
  ...
  expect(screen.getByLabelText('Today Sunny H:65° L:48°').querySelector('[class*="heroDayRangeTrack"]')).toHaveStyle({ '--range-marker': '52.94117647058824%', '--range-size': '100%', '--range-start': '0%' })
    ...
    expect(precipitationTile.querySelector('[data-probability="94"][data-amount="0"]')).toHaveAttribute('data-measurable', 'false')
    expect(within(precipitationTiles).getAllByRole('row')).toHaveLength(7)
    ...
    expect(humidityTile.querySelector('[data-precipitation-y-axis="humidity"]')).toHaveTextContent('80%70%60%')
    expect(cloudTile.querySelector('[data-precipitation-y-axis="cloud"]')).toHaveTextContent('70%35%0%')
    expect(humidityTile.querySelector('[data-hourly-metric-plot="humidity"]')).toHaveAttribute('data-axis-truncated', 'true')
