...
import {
  ...
  SCENARIO_IDS, SHARED_OWNER_ROOTS, SURFACE_CONTRACTS, type ContextId, type ScenarioId,
...
export function classifyChanges(changes: string[]) {
  ...
  if (!source.length) {
    ...
    return {
      mode: docsOnly || !changes.length ? 'non-layout' as const : 'tooling' as const,
      scenarios: docsOnly || !changes.length ? [] : [...SCENARIO_IDS],
      reasons: [docsOnly ? 'Documentation-only; no runtime or validation contract changes' : !changes.length ? 'No changed files' : 'Validation/tooling changes require representative workflow proof'],
      unowned: [],
    }
  }
  ...
  for (const file of source) {
    ...
    if (!owners.length && !shared.includes(file)) unowned.push(file)
  ...
  return { mode: 'focused' as const, scenarios: [...scopes], reasons: ['Explicit source owners; every declared state is retained'], unowned }
}
