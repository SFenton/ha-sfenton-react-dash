...
For an SSH deployment, `npm run deploy:both` builds the app and compares the panel, authenticated Admin To-Do image endpoint, and authenticated Home MCP proxy files first. Changed HA package/component files are backed up, staged, and configuration-checked before any React asset is replaced. When those files changed, the command stops after staging with exit status 2; restart Home Assistant with separate approval, verify the affected integration or package, then rerun to upload `dist/` and synchronize the legacy wrapper URL. Failed staging/validation attempts restore the prior configuration files. It supports the configured password or `VITE_SSH_PRIVATE_KEY`; on this workstation it defaults to `~/.ssh/ha-sfenton-react-dash-deploy`.
...
4. Back up and copy changed `home-assistant/custom_components/sfenton_admin_todo/` and `home-assistant/packages/sfenton_admin_todo.yaml`, plus `home-assistant/custom_components/sfenton_home_mcp_proxy/`, `home-assistant/packages/sfenton_home_mcp_proxy.yaml`, and `home-assistant/packages/solo_trip.yaml`. The one-time removal of the retired chat package/component is separately backed up and staged as documented in `docs/deployment.md`; asset sync never removes it.
...
6. Restart Home Assistant only with explicit approval when the panel package/bridge, Admin To-Do image integration, Home MCP proxy, Household Away integration, or retired chat runtime removal changed.
7. Verify the retired `sfenton_react_chat.purge_expired_history` service is absent after removal without invoking it; preserve existing user-history data. Verify authenticated admin-only Admin To-Do image behavior, authenticated `/api/sfenton_home_mcp` `home_info` and the pinned-TLS MCP container, and `sensor.household_away_status` plus `script.household_away_command` only after the native Solo Trip package passes its broker, journal, presence, and single-writer gates.
8. Install or update the separately owned Wake Light integration before Household Away when its source-suspension contract changed. Copy the clean `dist/` only after required runtime checks, then run `npm run deploy:sync`. React no longer enables Home MCP chat.

React assets, `deploy:sync`, and HMR cannot unload a previously loaded Python integration or activate a newly installed proxy. Keep the independent Home MCP tools and authenticated HA proxy healthy; do not re-enable frontend chat or its automatic feedback worker. See `docs/deployment.md` and `home-mcp/README.md` for the remaining release and rollback contracts.
