...
- Do not rely on browser-default blue focus outlines or add pointer-induced white borders, outlines, or focus shadows to inputs, textareas, selects, or custom picker shells. Text-entry controls can match `:focus-visible` after a mouse click or touch tap; that selector alone does not establish keyboard modality. Keep pointer-focused chrome neutral on both coarse and fine pointers. Preserve intentional keyboard focus: text entry may use a clearly visible native caret/selection, or an established keyboard-only treatment with explicit modality handling. Do not globally erase focus indicators or state-derived/error borders.
- On dashboard controls, do not leave persistent white focus or selected borders solely after a click or tap. This includes native dropdown/select wrappers, GlassTiles, buttons, chips, toggles, and modal controls.
- Textareas must hide scrollbar chrome without disabling overflow scrolling, editing, selection, or caret reveal. Apply cross-browser scrollbar rules to the actual textarea, including native appearance where necessary; do not substitute `overflow: hidden` or grow a stable composer to fit long text.
- Validate changed text-entry controls in mocked Playwright with actual click and touch tap, plus keyboard Tab entry. Compare computed input and wrapper outline, border, and box-shadow before/after pointer focus; retain error borders. Exercise pasted unbroken strings and multiline overflow, scrolling to both ends and editing at the caret, with unchanged control geometry and no visible scrollbar. Cover fine-pointer Chromium and touch Chromium/WebKit. Static design checks do not prove these browser behaviors.
