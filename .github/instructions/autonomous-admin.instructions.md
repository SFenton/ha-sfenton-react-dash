...
- Accepted completion must call the HA-owned `script.complete_admin_todo_item`; do not duplicate its notification side effect in Node or React.
...
- Hold one exclusive repository run lock for the entire launcher process so parallel runners cannot duplicate work, email, or notifications.
