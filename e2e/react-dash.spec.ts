...
async function openRoomsFromQuickLinks(scope: FrameLocator | Page) {
  const quickLinksDialog = await openQuickLinksTab(scope)
...
test('Food & Recipes hub and recipe browse use approved 393px mobile geometry', async ({ page }) => {
  ...
  await expect(globalQuickLinksAction(page)).toBeHidden()
...
test('overview renders with mock Home Assistant state', async ({ page }) => {
  ...
  await expect(globalQuickLinksAction(page)).toBeVisible()
})
...
test('global Quick Links opens from a non-Home route and preserves one-sheet detail navigation', async ({ page }) => {
  ...
  let dialog = await selectQuickLinksTab(page)
  await expect(dialog).toBeVisible()
  ...
  await trigger.click()
  dialog = await selectQuickLinksTab(page)
...
test.describe('desktop modal layout', () => {
  ...
  test('room-source vacuum zones tab scrolls above its constrained desktop modal nav', async ({ page }) => {
    ...
    const zonesPane = dialog.getByRole('group', { name: 'Main Floor controls, rooms, auto-clean, actions, info' })
  ...
  test('desktop modal preserves its size during the close fade', async ({ page }) => {
    ...
    const { frames, closingAtStart } = await dialog.evaluate(async (dialogElement, closeWindowMs) => {
      const closeButton = dialogElement.querySelector<HTMLButtonElement>('button[aria-label="Close"]')
      if (!closeButton) throw new Error('Rooms modal requires its mounted Close control')

      const samples: Array<{ height: number; width: number }> = []
      let closingAtStart: string | null = null
      const start = performance.now()
      closeButton.click()

      await new Promise<void>((resolve) => {
        const sample = () => {
          if (!document.body.contains(dialogElement) || performance.now() - start > closeWindowMs) {
            resolve()
            return
          }

          if (!samples.length) closingAtStart = dialogElement.getAttribute('data-closing')
      ...
      return { frames: samples, closingAtStart }
    }, 520)

    expect(closingAtStart).toBe('true')
...
test('room vacuum cards open reusable vacuum modal controls', async ({ page }) => {
  ...
  await expect(page.getByText('No rooms are selected, and no areas are drawn. If you begin cleaning, the robot vacuum will attempt to clean every mapped area.')).toBeVisible()
...
test('vacuum area editor draws, moves, resizes, zooms, and sends exact script geometry', async ({ page }) => {
  ...
  await dialog.getByRole('group', { name: 'Cleaning target' }).getByRole('button', { name: 'Area' }).click()
  await expect(dialog.getByText('No area selected')).toBeVisible()
  ...
  const mainMapOverlay = dialog.locator('[data-map-editor-overlay="true"]')
