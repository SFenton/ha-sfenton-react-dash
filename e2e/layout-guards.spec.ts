...
guardedTest('declared read-only body requires one real noninteractive state terminal', async ({ page }) => {
  ...
  await page.locator('#state-terminal').evaluate(node => { node.style.opacity = '1'; node.setAttribute('tabindex', '-1') })
  expect((await modalFacts(dialog, 'body', '#state-terminal')).terminalKind).toBe('read-only-content')
  await page.locator('#state-terminal').evaluate(node => { node.setAttribute('tabindex', '0') })
  await expect(modalFacts(dialog, 'body', '#state-terminal')).rejects.toThrow('visible, named, noninteractive')
  await page.locator('#state-terminal').evaluate(node => { node.removeAttribute('tabindex'); node.innerHTML = '<div contenteditable="true">Editable</div>' })
  await expect(modalFacts(dialog, 'body', '#state-terminal')).rejects.toThrow('visible, named, noninteractive')
  await page.locator('#state-terminal').evaluate(node => { node.innerHTML = '<details><summary>More</summary><p>Details</p></details>' })
  await expect(modalFacts(dialog, 'body', '#state-terminal')).rejects.toThrow('visible, named, noninteractive')
  await page.locator('#state-terminal').evaluate(node => { node.innerHTML = '<button>Unclassified action</button>' })
