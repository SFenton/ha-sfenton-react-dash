...
import { publishHomeMcp, restoreSourceLink, switchSourceLink } from './publish'
...
describe('Home MCP source switching', () => {
  ...
  it('refuses unrelated deployment roots before any filesystem or network access', async () => {
    await expect(publishHomeMcp('.', '/tmp/unrelated-deployment')).rejects.toThrow('unsafe Home MCP deployment root')
  })
})
